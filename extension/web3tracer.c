...
static void web3tracer_add_in(uint32 call_no,const char *func_name,const char *class_name, int internal_user,const char *include_file,const char *call_file, uint call_line_no TSRMLS_DC);
...
PHP_FUNCTION(web3tracer_enable) {
	...
	if (!WEB3TRACER_G(enabled)) {
		...
		web3tracer_add_in(0, WEB3TRACER_ROOT_SYMBOL, NULL, 0, NULL, NULL, 0 TSRMLS_CC);
...
#define web3tracer_cg_stop(prop,amt)																	\
	...
	if(WEB3TRACER_G(lastCall)->cycleSource!=WEB3TRACER_G(lastCall)){																\
		...
		for(callCursor=WEB3TRACER_G(lastCall)->cycleSource->next;callCursor!=WEB3TRACER_G(lastCall);callCursor=callCursor->next){	\
			if(!callCursor->drop){																		\
...
static void web3tracer_add_out(uint32 call_no TSRMLS_DC){
	...
	if(!WEB3TRACER_G(lastCall)->drop){
		...
		web3tracer_add_output_internal(WEB3TRACER_G(lastCall)->fn,(uint64)(1000*get_us_from_tsc(time.internalAmount,WEB3TRACER_G(cpu_frequencies)[WEB3TRACER_G(cur_cpu_id)]) TSRMLS_CC),mmax.internalAmount,mall.internalAmount,mfre.internalAmount,mvar.internalAmount);
		if(WEB3TRACER_G(lastCall)->cycle){
...
static int web3tracer_in(int internal_user TSRMLS_DC) {
	...
	if (EG(current_execute_data) &&
		...
		snprintf(WEB3TRACER_G(fname2),WEB3TRACER_FNAME_BUFFER,"{closure}%s:%d",EG(current_execute_data)->function_state.function->op_array.filename,EG(current_execute_data)->function_state.function->op_array.line_start);
		function=WEB3TRACER_G(fname2);
	} else {
